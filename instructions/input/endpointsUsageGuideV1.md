...
**Usage:**
Call this endpoint after login or on page refresh to get the user's current data. Can also be used to check validation of access token.

**Behavior:**

- Requires a valid `accessToken`.
- Returns the full user object from the database, including all associated profiles and roles.
- ok response means token is valid.
