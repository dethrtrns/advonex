...
import { VerifyOtpEmailResponse } from "@/lib/types/types";
...
export async function sendOtp(params: RequestOtpParams): Promise<void> {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_BACKEND_URL}/auth/request-otp`,
      {
        method: "POST",
        headers: {
...
export async function verifyOtp(
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_BACKEND_URL}/auth/verify-otp`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
